...
TODO: tableau : composant, fonction, fonctionnement.
...
Puisque $b \ll $ 10 GeV, aux énergies qui nous intéressent :

\begin{equation}
\dfrac{\sigma_E}{E} \sim \dfrac{a}{\sqrt{E}} \oplus c
\end{equation}

A l'aide des simulations Monte-Carlo, on estime la résolution en énergie en fonction de l'énergie des photons mesurés, afin de vérifier si ce comportement est bien reproduit. (fig. \ref{fig:resolution_energie_photons}).
...
\begin{equation}
\Delta m_{\gamma\gamma} \simeq \dfrac{1}{2}m_{\gamma \gamma}\left( \dfrac{\sigma_{E_1}}{E_1} +  \dfrac{\sigma_{E_2}}{E_2}\right) + \dfrac{E_1E_2 \sinh {(\eta_1 - \eta_2)}}{m_{\gamma \gamma}} \left( \sigma_{\eta_1} + \sigma_{\eta_2} \right)
\end{equation}

Si un ordre de grandeur suffit :

\begin{equation}
\Delta m_{\gamma\gamma} \sim m_{\gamma \gamma}\left ( \dfrac{\sigma_E}{E} +  \sigma_{\eta} \right )
\end{equation}
...
\begin{equation}
\int L dt = \dfrac{N}{\xi \sigma} = \dfrac{\mbox{900}}{\mbox{0,41} \times \mbox{51} \times \mbox{10} {}^{-15}} = \mbox{21 fb} {}^{-1} \pm 20 \%
\end{equation}
...
\begin{tabular}{|l|c|r|} 
  ...
  \hline
   $\sigma_Z$ & 8,5 $\pm$ 1,6 GeV & 2,5 GeV \\
 \hline
\end{tabular}

La masse ainsi calculée est correcte. La largeur est bien plus importante que la valeur théorique. On s'attend à une largeur de la forme $\sigma_{exp} \sim \sqrt{(\Delta m_{\gamma \gamma})^2 + \sigma_{SM}^2}$ qui aurait une valeur d'environ 4 GeV. 

TODO: expliquer différence !

\subsection{Recherche de Higgs à des masses différentes }

En connaissant les sections efficaces et ratios de branche du Higgs en fonction de sa masse, et en extrapolant le fit de bruit de fond mesuré à des masses élevées, on peut estimer le ratio signal/bruit de fond pour des scénarios avec un boson de Higgs plus lourd. Cela permet de montrer que le canal $H \to \gamma \gamma$ n'est pas approprié à la détection de Higgs lourd. Sur la figure  \ref{higgs_lourds_gg}, on peut en effet constater que le ratio S/B ne dépasse pas 0,01. De plus, le run 1 du LHC n'aurait pas produit plus de 8 évènements $H \to \gamma \gamma$ pour $m_H =$ 200 GeV (et aucun pour 300+GeV).
\begin{figure}[H]
\centering
